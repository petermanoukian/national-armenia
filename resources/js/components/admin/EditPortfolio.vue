			...
			<div class="card">
				<div class="card-header">
					...
					<div class="card-body">
						...
						<div> 
						 <span v-for="item, index in port.tag " > 58 {{ item }} {{ item.id }}  </span>
						...
						<div> HERE C TAGX
						 <span v-for="item, index in tagx " > 63 {{ item }} {{ item.id }}  </span>
						...
						<div class="form-group row">
							<ckeditor :editor="editor" v-model="port.description" :config="editorConfig" ></ckeditor>		
						</div>	
						...
						<div class="form-group row">
							   <div class="col-md-3" >
								  <img :src=imagepath class="img-responsive" height="120" width="120" v-if="image1">
								  <img :src="imagex" class="img-responsive" height="120" width="120" v-if="image2">
							   </div>
        ...
        methods: 
		{
           ...
           editport() 
		   {
			...
			//alert(id);
				...
				{  
                    this.tags2 = res;
					console.log('READY 320 tags are ' + res.data);
					console.log('READY 321 tags are ' + res.data);
                });
			...
			fetchPortArray(id) {
                ...
                axios.get(`/admin/tags/jsontagArray/${id}`).then((res) => 
				{  
                    this.port.tag = res.data;
					//this.tags = res.data.id;
					this.tagx = res.data;
					console.log('332 tags are ' + res.data);
					console.log('333 tags are ' + res.data);
                });
